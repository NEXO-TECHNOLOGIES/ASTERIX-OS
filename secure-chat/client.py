...
class ChatClient:
    ...
    def join_room(self, room, password, name):
        ...
        try:
            ...
            if err_data.get("intruder_detected"):
                print(f"\n{C_RED}{C_BOLD}[ALERT] [!] THIEF ALERT: 3+ failed password attempts! Target IP flagged to OS defense.{C_RESET}\n")
            return False, err_data.get("error", str(e))
        except Exception as e:
            return False, str(e)
    ...
    def handle_event(self, payload):
        ...
        if evt_type == "peer_joined":
            ...
            print(f"\n  {C_YELLOW}{C_BOLD}[[FIRE]] Chat history purged by administrator.{C_RESET}")
            print(f"  {C_GREEN}user  {C_RESET}", end="", flush=True)
        elif evt_type == "intrusion_alert":
            # [ALERT] SNOOPER CAUGHT
            ...
            burn_str = f" {C_YELLOW}[[FIRE] {burn}s]{C_RESET}" if burn > 0 else ""
    ...
    def send_message(self, text, burn_secs=0):
        ...
        try:
            ...
            print(f"  {C_RED}[[FAIL]] Send error: {err.get('error')}{C_RESET}")
    ...
    def list_members(self):
        ...
        try:
            ...
            print(f"  {C_RED}[[FAIL]] Could not list members: {e}{C_RESET}")
    ...
    def kick(self, target_id):
        ...
        try:
            with urllib.request.urlopen(req, timeout=10) as resp:
                print(f"  {C_GREEN}[[OK]] Operative kicked successfully.{C_RESET}")
        except Exception as e:
            print(f"  {C_RED}[[FAIL]] Kick failed: {e}{C_RESET}")
    ...
    def mute(self, target_id, muted=True):
        ...
        try:
            with urllib.request.urlopen(req, timeout=10) as resp:
                print(f"  {C_GREEN}[[OK]] Operative mute state updated.{C_RESET}")
        except Exception as e:
            print(f"  {C_RED}[[FAIL]] Mute failed: {e}{C_RESET}")
    ...
    def purge(self):
        ...
        try:
            with urllib.request.urlopen(req, timeout=10) as resp:
                print(f"  {C_GREEN}[[OK]] All vault messages purged.{C_RESET}")
        except Exception as e:
            print(f"  {C_RED}[[FAIL]] Purge failed: {e}{C_RESET}")
...
def main():
    ...
    if choice == "1":
        ...
        if not ok:
            print(f"\n  {C_RED}[[FAIL]] Room creation failed: {msg}{C_RESET}\n")
            return
        print(f"\n  {C_GREEN}{C_BOLD}[[OK]] VAULT CREATED (ADMIN PRIVILEGES ENGAGED){C_RESET}")
    else:
        ...
        if not ok:
            print(f"\n  {C_RED}[[FAIL]] Authentication failed: {msg}{C_RESET}\n")
            return
        print(f"\n  {C_GREEN}{C_BOLD}[[OK]] AUTHENTICATED INTO VAULT ROOM:{C_RESET} {C_WHITE}{room}{C_RESET}")
