...
if [ -d /tmp ]; then
    ...
    echo -e "  ${C_GREEN}[[OK]] /tmp cleared${C_RESET}"
fi

# Wipe /var/tmp
if [ -d /var/tmp ]; then
    find /var/tmp -mindepth 1 -mtime +1 -delete 2>/dev/null || true
    echo -e "  ${C_GREEN}[[OK]] /var/tmp stale artifacts purged${C_RESET}"
fi
...
if [ "$EUID" -eq 0 ]; then
    ...
    echo 3 > /proc/sys/vm/drop_caches 2>/dev/null && \
        echo -e "  ${C_GREEN}[[OK]] RAM Page Cache Flushed (Pagecache + Dentries + Inodes)${C_RESET}" || \
...
if [ -f "${HOME}/.bash_history" ]; then
    ...
    echo -e "  ${C_GREEN}[[OK]] Bash history cleared${C_RESET}"
fi
...
if [ -d "$THUMB_DIR" ]; then
    ...
    echo -e "  ${C_GREEN}[[OK]] Thumbnail cache wiped${C_RESET}"
fi
...
if [ -d "$ASTERIX_LOGS" ]; then
    ...
    echo -e "  ${C_GREEN}[[OK]] ASTERIX session logs shredded${C_RESET}"
fi

echo ""
echo -e "${C_GREEN}${C_BOLD}[[OK]] System cleanup complete. Approximate disk space freed: ${BYTES_FREED} bytes${C_RESET}"
