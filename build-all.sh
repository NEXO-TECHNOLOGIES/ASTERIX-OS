...
if command -v gcc &>/dev/null; then
    ...
    echo -e "${C_GREEN}[[OK]] C suite done${C_RESET}"
else
...
if command -v nasm &>/dev/null && command -v ld &>/dev/null; then
    ...
    make all
    sudo make install DESTDIR="$INSTALL_DIR" 2>/dev/null || make install DESTDIR="${SCRIPT_DIR}/dist"
    echo -e "${C_GREEN}[[OK]] Assembly tools done${C_RESET}"
else
...
if command -v go &>/dev/null; then
    ...
    echo -e "${C_GREEN}[[OK]] Go suite done${C_RESET}"
else
    ...
    if [ -x "./build.sh" ]; then
        ...
        for eng in asterix-bin-inspector asterix-net-sentinel asterix-crypto-core asterix-sys-mon asterix-guard-engine asterix-dark-engine asterix-log-hunter asterix-code-repair; do
            if [ -f "target/release/${eng}" ]; then
                ...
                echo -e "${C_GREEN}  [[OK]] ${eng} installed${C_RESET}"
            fi
        done
    fi
fi
...
if [ -d "${SCRIPT_DIR}/ui-core/asterix-loader" ]; then
    ...
    if command -v cargo &>/dev/null; then
        ...
        echo -e "${C_GREEN}[[OK]] asterix-loader compiled via cargo${C_RESET}"
        ...
        echo -e "${C_GREEN}[[OK]] asterix-loader compiled via rustc${C_RESET}"
    else
...
if [ -f "${SCRIPT_DIR}/bin/ax" ]; then
    ...
    echo -e "${C_GREEN}[[OK]] Master 'ax' and 'asterix' CLI installed${C_RESET}"
fi

echo ""
echo -e "${C_GREEN}${C_BOLD}[[OK]] ASTERIX OS Master Build Complete!${C_RESET}"
